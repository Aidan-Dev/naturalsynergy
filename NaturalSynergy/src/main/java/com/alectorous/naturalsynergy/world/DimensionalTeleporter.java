...
public class DimensionalTeleporter extends Teleporter {
	...
	public DimensionalTeleporter(WorldServer world, double x, double y, double z) {
		super(world);
	...
	public static void teleportToDimension(EntityPlayer player, int dimension, double x, double y, double z) {
		...
		worldServer.getMinecraftServer().getPlayerList().transferPlayerToDimension(entityPlayerMP, dimension,
				new DimensionalTeleporter(worldServer, x, y, z));
		player.setPositionAndUpdate(x, y, z);
		
		//Have to treat leaving the end separately 
		if (oldDimension == 1) {
			player.setPositionAndUpdate(x, y, z);
